...
const ProjetHero = ({ title, imgHero, idProject, hrefWebsite }) => {
    ...
    return (
        <div className="h-screen 2xl:h-1000 3xl:h-screen 4xl:h-screen 5xl:h-[1200px] 6xl:h-[1250px] relative overflow-hidden">
            ...
            <div className="flex flex-col lg:flex-row py-48 lg:py-0 justify-center h-full items-center gap-8">
                <motion.div
                    ref={ref}
                    initial={prefersReducedMotion ? false : { opacity: 0, y: -100 }}
                    animate={isInView ? { opacity: 1, y: 0 } : {}}
                    transition={{ duration: prefersReducedMotion ? 0 : 0.5 }}
                    className="font-raleway basis-full max-w-90% h-auto lg:basis-2/5 lg:pl-8 flex flex-col justify-center lg:justify-start items-center"
                >
                    <div className="z-10 text-center lg:text-left max-w-full">
                        <p className="font-extrabold text-7xl sm:text-8xl md:text-9xl lg:text-10xl text-whiteprimary">0{idProject}.</p>
                        <h1 className="font-rubik font-bold uppercase text-5xl sm:text-6xl md:text-6xl lg:text-7xl text-whiteprimary break-words">{title}</h1>
                        <CustomLink
                            href={hrefWebsite}
                            target="_blank"
                ...
                <motion.div
                    ...
                    className="relative basis-full lg:basis-3/5 w-full h-full 3xl:h-[80%] flex items-center z-10 px-5 xs:px-0"
                >
